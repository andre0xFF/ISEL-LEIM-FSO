...
public class Trabalho04JFrame extends JFrame {
	...
	void initVariables() {
		...
		robotLego = new MyRobotLego(txtLog, false, robotPlayer);
	}
	...
	public Trabalho04JFrame() {
		...
		robotPlayer = new RobotPlayer(robotLego, this);
		robotPlayer.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		robotPlayer.setVisible(true);
	...
	public void setRobotConfig(String s) {
		if(s == null) log("Config for robot " + robotName + " not found");
		else {
			String[] config = s.split(":");
			offsetLeft = Integer.parseInt(config[0]);
			offsetRight = Integer.parseInt(config[1]);
			updateGuiComponents();
			log("Config for robot " + robotName + ": "+config[0]+":"+config[1]);
		}
	}
}
